...
function rotateLayer(axis, angle, position) {
  for (const i in axis) {
    if ((axis[i] === 1) || (axis[i] === -1)) {
      game.controls.keyboardMove('LAYER', { position: position, axis: i, angle: angle });
      break
    }
  }
}

function getTransformedAxis(axis, angle) {
  const quaternion = new THREE.Quaternion();
  quaternion.setFromAxisAngle(axis, angle);
  console.log(quaternion.setFromAxisAngle(axis, angle))
  const transformedAxis = axis.clone().applyQuaternion(quaternion);
  // console.log(transformedAxis)
  return transformedAxis;
}

function rotateU() {
  rotateLayer(new THREE.Vector3(0, 1, 0), -Math.PI / 2, new THREE.Vector3(0, 1, 0));
}
...
function rotateR() {
  rotateLayer(new THREE.Vector3(1, 0, 0), -Math.PI / 2, new THREE.Vector3(1, 0, 0));
}
...
function rotateF() {
  rotateLayer(new THREE.Vector3(0, 0, 1), -Math.PI / 2, new THREE.Vector3(0, 0, 1));
}
...
function rotateView(direction, isPrime = false) {
  const move = {
    'up': { axis: new THREE.Vector3(1, 0, 0), angle: isPrime ? Math.PI / 2 : -Math.PI / 2 },
    'down': { axis: new THREE.Vector3(1, 0, 0), angle: isPrime ? -Math.PI / 2 : Math.PI / 2 },
    'left': { axis: new THREE.Vector3(0, 1, 0), angle: isPrime ? -Math.PI / 2 : Math.PI / 2 },
    'right': { axis: new THREE.Vector3(0, 1, 0), angle: isPrime ? Math.PI / 2 : -Math.PI / 2 }
...
// Add event listener for mouse move to resume game if paused
